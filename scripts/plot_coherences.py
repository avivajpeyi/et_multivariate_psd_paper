...
def add_legend(axes):
    ...
    handles = [
        [
            plt.Line2D([0], [0], color='k', label='True', linestyle='-', lw=0.75),
            plt.Line2D([0], [0], color='k', label='Expected', linestyle='--', lw=0.75),
        ],
        [
            plt.Line2D([0], [0], color='C0', label='XY      ', lw=3),
            plt.Line2D([0], [0], color='C1', label='XZ      ', lw=3),
            plt.Line2D([0], [0], color='C2', label='YZ      ', lw=3),
        ]

    ]
    # make sure the two legned are horizontally aligned
    axes[0].legend(handles=handles[0], **kwgs)
    axes[1].legend(handles=handles[1], **kwgs)
...
def main():
    ...
    axes[0].set_yticks([1, 0.1, 0.008])
    axes[0].set_yticklabels(['1', '0.1', '.008'])

    axes[1].set_yticks([1, 0.1,  0.008])
    axes[1].set_yticklabels(['1', '0.1', '.008'])
