...
In order to avoid underfitting in SGVB, we determine an appropriate number of basis functions for each dataset using the method from Sec.~\ref{subsec:sgvb_details}. 
Fig.~\ref{fig:sim_basis} shows the log MLE (normalised for comparison between datasets) plotted against the number of basis functions.
We set $M=30$ as the log MLE appears stable by this point. 
...
\begin{figure}
  ...
  \captionof{figure}{
  Comparison of VNPC and SGVB methods for VAR(2) and VMA(1) models for different data lengths $n$. (a,b) Violin plots of $L_2$ errors from 500 realizations for VAR(2) and VMA(1) models respectively. (c) Speed-up factor of SGVB compared to VNPC.}
...
Furthermore, the simulation study provides empirical evidence that the \ac{SGVB} approach is consistent, i.e., with increasing sample size, the posterior distributions concentrate around the true spectral density ($L_2$ errors are $<0.3$ \avi{what does this actually mean? Do we have a scale to what is expected as 'good'}\kl{It's just the Frobenius norm (some distance for matrix values) on average. For a univariate series, it would be more straightforward.}). Refer to Appendix~\ref{appdx:simstudy} for more details on the results of the simulation study. 
...
\avi{Could there be some minute correlations that may arise from numerical artifacts or inherent properties of the simulated noise?} \NC{For a sample size of N in the two channels, and assuming the two channels are uncorrelated, the coherence will be about 1/N. That's probably what is seen here.}
