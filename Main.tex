...
\author{Jianan Liu$^{1}$}
\email{jliu812@aucklanduni.ac.nz}
...
\affiliation{$^{2}$Universiteit Antwerpen, Prinsstraat 13, 2000 Antwerpen, Belgium}
\affiliation{$^{3}$Universit\'e C$\hat{o}$te d’Azur, Observatoire C$\hat{o}$te d’Azur, CNRS, Artemis, 06304 Nice, France}
...
\affiliation{$^{6}$Department of Mathematical Sciences, Auckland University of Technology, Auckland, New Zealand}
...
Due to the normalizing and decorrelating properties of the Fourier transform, the discrete Fourier coefficients $\d(f_k)$ %(multiplied by $1 / \sqrt{n}$)
...
\begin{align}
\Re(\theta_{jl}^{(k)}) &= \alpha_{jl,0} + \alpha_{jl,1}f_k + \sum_{s=1}^{M-1}\psi(f_k)\alpha_{jl,s+1}, \label{eq:real_theta} \\
\Im(\theta_{jl}^{(k)}) &= \beta_{jl,0} + \beta_{jl,1}f_k + \sum_{s=1}^{M-1}\psi(f_k)\beta_{jl,s+1}, \label{eq:imag_theta} \\
\log \delta_{jk}^2 &= \gamma_{j,0} + \gamma_{j,1}f_k + \sum_{s=1}^{M-1}\psi(f_k)\gamma_{j,s+1},  \label{eq:log_delta}
\end{align}
...
% The specific code version used, along with the the raw data and data products for this work have been archived on Zenodo~\cite{}.
