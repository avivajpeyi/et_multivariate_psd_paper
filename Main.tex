...
Quantifying the frequency-dependent relationship between time series is crucial in many applications\citeme. 
...
\begin{figure}
  ...
  \captionof{figure}{
  The relationship between the number of basis functions ($M$) and the normalized maximum likelihood estimate (MLE) for VAR(2) and VMA(1) models (solid and dotted lines respectively) with different data lengths $n=256$ (blue), $n=512$ (green), and  $n=1024$ (orange). 
  \avi{Is this likelihood or log likelihood? MLE stannds for max likelihood estimate, not max loglike estimate}
  }
  \label{fig:sim_basis}
\end{figure}%
...
\begin{figure}
  ...
  \avi{We should change the xlabel to $n$.}
  }
...
Following the methodology of \citet{Janssens2023},
we inject an additional set of colored Gaussian noise to simulate non-identical correlated noise across the X, Y, and Z channels. 
This noise is characterized by Gaussian peaks in the frequency domain, defined by the power spectral density: 
\begin{equation}
...
\end{itemize}
Note that to induce correlated noise between channel pairs, we use identical data for the Gaussian peaks at the same frequencies. 
For uncorrelated noise, we simulate Gaussian peaks for each channel. 

Although we recognize that this dataset is simplified and highly separable due to the unique nature of the correlated noise terms, it serves as an initial demonstration. 
In future work, we will increase complexity to approach more realistic noise scenarios, including the incorporation of correlated magnetic and/or Newtonian noise.
...
\centering
  ...
  \caption{Relationship between the number of basis functions ($M$) and the normalized maximum likelihood estimate (MLE) for different scenarios of Einstein Telescope noise analysis. Case A (blue line) represents correlated noise, while Cases B and C (green line) represent uncorrelated noise scenarios.
  \avi{is this likelihood or log likehood}
  }
  \label{et_corr_basis_funs_vs_mle}
\end{figure}


We conduct three analyses to estimate the PSD and the corresponding squared coherences under different noise conditions: 
\begin{itemize}
    \item[] \textbf{Case A:}  ET noise with correlated Gaussian peaks  as described in Section~\ref{sec:data_gen},
    \item[] \textbf{Case B:} ET noise with uncorrelated Guassian peaks,
    \item[] \textbf{Case C:} ET noise with uncorrelated Gaussian peaks, assuming independence between the channels.
\end{itemize}
...
Each Whittle likelihood block, corresponding to a specific input segment, utilizes identical basis function expressions for the spectral density matrix. 

Figure~\ref{et_corr_basis_funs_vs_mle} displays the MLE versus the number of basis functions for each case. 
Observing the MLE stabilising as the number of basis functions increase, we set the number of basis functions to be $M=450$.
Using the methodology from Section~\ref{sec:method} we estimate the PSD for each case and plot the PSD in Figure~\ref{fig:ET_PSDS}.
...
\begin{figure}
  ...
  \caption{Squared coherence estimates for cases A (top) and B (bottom). }
...
The analysis of the three cases reveals distinct spectral characteristics across the X, Y, and Z channels of the Einstein Telescope (ET) noise simulations. 
In all three scenarios, we observe prominent spectral peaks at specific frequencies in each channel, consistent with the injected Gaussian noise peaks.
...
Additionally, investigating the method's performance on GPUs could further validate its robustness and versatility.
...
\begin{figure}[h]
  ...
  \caption{Blocked Whittle likelihood as a function of the number of blocks utilised. \avi{The top needs to read as block lenngth}}
...
Figure~\ref{fig:lnl_vs_nchunks} illustrates the relationship between the number of blocks used in our blocked Whittle likelihood approach and the ``blocked'' likelihood value, relative to the likelihood with one block (the traditional whittle likelihood). 
The data used for the likelihood is from \avi{Jianan fill this..}..
The model used ... talk about it being the same for all the likelihood evaluation.

As the number of blocks increases, we observe a consistent decrease in the estimated likelihood.
