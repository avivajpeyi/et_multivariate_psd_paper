...
%% Duplicated below
% The SGVB approach is applied to simulated ET noise, demonstrating its flexibility and applicability to other detectors like LISA and the LIGO-Virgo-KAGRA (LVK) network. A blocked Whittle likelihood approximation is introduced to manage large time series \citep{vu2024}.
% Guidance on SGVB tuning parameters, such as learning rates and basis functions, is provided. 
% In addition, the coherence is computed as a function of frequency, quantifying correlations in the multivariate time series.
...
%\avi{Kate will take a crack as this paragraph}
...
%It is crucial to keep $M$ below 50\% of the sample size $n$ to avoid overfitting\footnote{This upper limit ($M \lesssim n/2$) is empirically determined and can be adjusted if necessary.}. 

%\avi{add note on the prior helping prevent overfitting}

% \jliu{As the number of basis functions $M$ increases, the maximum likelihood estimate (MLE) initially rises sharply, reflecting a substantial improvement in model fit as the model complexity increases. However, as $M$ continues to increase, the MLE exhibits a more gradual and steady rise, indicating that while the model is still improving, the rate of improvement has slowed as it captures the underlying structure of the data. Once $M$ reaches a sufficiently large value where the improvements become marginal, further increases in $M$ lead to fluctuations in the MLE, suggesting that the model is starting to overfit, with the added complexity introducing bias rather than improving the fit}. \jliu{When M continues to increase, the trend of MLE will be disturbed, so a large M will have a bias on the value of MLE. This phenomenon can be found on page 33 of Google doc} 
...
$M=30$ is set as the log MLE enters a steady ascent phase and before any perturbations appear, thereby avoiding both underfitting across the three different dataset sizes. \kl{You can comment on the blue line something like this. When $n=256$, a rapid increase of the log-likelihood is observed as $M>50$, and this is clearly a sign of overfitting; the number of parameters ($(M+1)4$ for bivariate time series) exceeds the number of data points.}  
\avi{Say somthing about $n=256$, M increases but we select a lower region becasue overfitting, i think we're selecting}\jliu{what does the $(M+1)4$ mean?}.
...
\begin{figure}
  ...
  \captionof{figure}{
  Comparison of VNPC and SGVB methods for VAR(2) and VMA(1) models for different data lengths $n$. (a,b) Violin plots of $L_2$ errors from 500 realizations for VAR(2) and VMA(1) models respectively. (c) Speed-up factor of SGVB compared to VNPC. The SGVB method uses optimized values for $M$ and $\tau_1$.}
...
\avi{state that we choose this to get a freq resolution size of X.} \jliu{Given that all Gaussian peaks across the channels are concentrated within the first 128 Hz, only the data corresponding to these frequencies is extracted from each block, resulting in a frequency resolution of 2048 points.}
...
\avi{highlight flexibility of technique, can take on any shape of the PSD, unlike a parametric approach. }
...
% For each realization, the traditional log Whittle likelihood (Equation~\ref{eq:Whittle likelihood}) is calculated by using the entire dataset. 
% Subsequently, each dataset is partitioned into varying numbers of blocks to compute the blocked log Whittle likelihood (Equation~\ref{eq:block_lnl}) for each configuration. 
% The median and 90\% credible interval blocked log likelihood is then determined across the 50 realizations for each block configuration.

% Figure~\ref{fig:lnl_vs_nchunks} illustrates the inverse relationship between the number of blocks and the error of the blocked likelihood, relative to the traditional Whittle likelihood (one block). 
% As the number of blocks increases, the relative error consistently increases.

% \begin{figure}
%   \includegraphics[width=\columnwidth]{relative_error_vs_nchunks}
%   \caption{Relative error of the \avi{log(?)} Blocked Whittle likelihood as a function of the number and length of blocks utilised. The solid green curve represent the median values, while the shaded areas indicate the corresponding 90\% credible interval.}
%   \label{fig:lnl_vs_nchunks}
% \end{figure}

% Even though partitioning the data into blocks can dramatically reduce the computational cost, a high number of blocks implies a lower number of observations per block, reducing the information on overall spectral characteristics and potentially losing long-range correlations and low-frequency information. 
% More blocks introduce additional boundaries, creating discontinuities that may lead to spectral leakage and reduced accuracy in spectral density estimation. 
% Furthermore, smaller block sizes decrease the frequency resolution, potentially missing fine spectral features.




%This trend highlights the need for an optimal balance between computational efficiency (favoring more blocks) and estimation accuracy (favoring fewer blocks). This trade-off is crucial in gravitational wave data analysis, where both speed and precision are essential. \pmr{What is the advise?}
%Since the relative error varies across datasets, we recommend examining the relative error as the number of blocks increases and selecting a block size where the error approaches zero. 
